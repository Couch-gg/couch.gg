...
| 3A | Games registry + REST + admin (+tests) | 3 | Opus | apps/realtime/src/persistence.ts, apps/realtime/src/games-registry.ts (new), apps/realtime/src/index.ts, apps/realtime/src/external-games.ts, apps/realtime/tests/games-registry.test.ts (new), .env.example | IN-FLIGHT | 0 | pinned: sha256 token, timingSafeEqual, no-TTL Redis keys, in-memory fallback off-Vercel, probe injectable |
